...
ensure the image of each capture area to align
...
capture areas to align come from different slides. `rescale_fiji_inputs()` reads in the
...
Before building a `SpatialExperiment` for this dataset, we must align the images for each
...
Next, `prep_fiji_image()` will rescale the stitched image to have a default of 1200 pixels
...
spots will not be shown. We'll also mirror the image and gene-expression data to
