...
`add_array_coords()` (which is used internally by `build_spe()`) recomputed the
...
The `build_spe()` function retains each capture area's original array
...
them with `build_spe(calc_error_metrics = TRUE)`. The latter metric can take a
